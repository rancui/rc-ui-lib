...
import { FormContenxt } from '@/components/form';
...
const Field = forwardRef<unknown, FieldProps>((props, ref) => {
        ...
        rules.reduce(
            (promise, rule) =>
                promise.then(() => {
                    ...
                    if (rule.validator) {
                        return (runValidator(current, rule) as any).then((result) => {
    ...
    const handleKeypress: KeyboardEventHandler<HTMLInputElement | HTMLTextAreaElement> = (
        event
    ) => {
        if (event.key === 'Enter') {
    ...
    const renderInput = () => {
        ...
        if (type === 'textarea') {
            return (
                <textarea ref={inputRef as MutableRefObject<HTMLTextAreaElement>} {...inputProps} />
            );
        }
        ...
        >
            <div className={classnames(`${baseClass}__body`)}>
                ...
                {showClear && (
                    <Icon
                        name="clear"
                        className={classnames(`${baseClass}__clear`)}
                        onClick={handleClear}
                    />
                )}
                {renderRightIcon()}
