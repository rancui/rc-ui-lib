...
const Form = forwardRef<unknown, FormProps>((props, ref) => {
  ...
  const validateSeq = (names?: string[]) => {
    return new Promise<void>((resolve, reject) => {
      ...
      const fields = getFieldsByNames(names) as Array<any>;
      fields
        .reduce(
          (promise, field) =>
            promise.then(() => {
              if (!errors.length) {
                return field.ref?.current?.validate().then((error?: FieldValidateError) => {
                  if (error) {
                    errors.push(error);
                  }
                });
  ...
  const getValues = () => {
    return (React.Children.toArray(children) as Array<any>).reduce((form, item) => {
  ...
  const resetValidation = (name?: string | string[]) => {
    ...
    const fields = getFieldsByNames(name) as Array<any>;
    fields.forEach((item) => {
